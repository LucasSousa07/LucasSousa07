...
class  UsuarioDAO
{
    ...
    {
        $query = "SELECT * FROM techjobsdb.usuario";
    ...
    public function getById(int $id)
    {
        ...
        $query = "SELECT * FROM techjobsdb.usuario WHERE id = :id;";
    ...
    public function insert($nome, $razao_social, $cpf, $cnpj, $senha, $email, $sexo, $ddi, $ddd, $telefone, $cep, $cidade, $logradouro, $bairro, $rua, $complemento, $perfil)
    {
        ...
        $query = "INSERT INTO techjobsdb.usuario (nome, razao_social, cpf, cnpj, senha, email, sexo , ddi, 
    ...
    public function update($id, $email, $senha, $nome, $perfil): int
    {
        ...
        $query = "UPDATE techjobsdb.usuario SET nome = :nome, email = :email, senha = :senha, perfil_id = :perfil_id WHERE id = :id";
        $stmt = $this->dbh->prepare($query);
        $stmt->bindParam(":nome", $nome);
        $stmt->bindParam(":email", $email);
        $stmt->bindParam(":senha", $senha);
    ...
    public function delete($id)
    {
        ...
        $query = "DELETE FROM techjobsdb.usuario WHERE id = :id";
        $stmt = $this->dbh->prepare($query);
        $stmt->bindParam(":id", $id);
    ...
    public function validarLogin($email, $senha)
    {
        ...
        $query = "SELECT id, nome, email, perfil_id FROM techjobsdb.usuario 
    ...
    public function processarEnvioEmail($email)
    {
        ...
        $query = "SELECT email FROM techjobsdb.usuario WHERE email = :email";
        $stmt = $this->dbh->prepare($query);
        $stmt->bindParam(':email', $email);
        $stmt->execute();
    ...
    public function senhaUpdate($senha, $id)
    {
        ...
        $senha = filter_input(INPUT_POST, 'senha', FILTER_SANITIZE_SPECIAL_CHARS);
        $query = "UPDATE techjobsdb.usuario SET  senha = :senha WHERE id = :id";
        $stmt = $this->dbh->prepare($query);
        $stmt->bindParam(":senha", $senha);
        $stmt->bindParam(":id", $id);
